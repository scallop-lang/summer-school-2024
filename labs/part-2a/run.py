...
class HWFDataset(torch.utils.data.Dataset):
  def __init__(self, root: str, prefix: str, split: str):
    ...
    self.root = root
    self.split = split
    self.metadata = json.load(open(os.path.join(root, f"{prefix}_{split}.json")))
  ...
  def __getitem__(self, index):
    ...
    for img_path in sample["img_paths"]:
      img_full_path = os.path.join(self.root, "symbols", img_path)
...
class HWFNet(nn.Module):
  def __init__(self, no_sample_k, sample_k, provenance, k, debug=False):
    ...
    # Scallop context
    self.scallop_file = "hwf.scl" if not args.do_not_use_hash else "hwf_parser_wo_hash.scl"
    self.symbols = [str(i) for i in range(10)] + ["+", "-", "*", "/"]
    self.ctx = scallopy.ScallopContext(provenance=provenance, k=k)
    self.ctx.import_file(os.path.abspath(os.path.join(os.path.abspath(__file__), f"../{self.scallop_file}")))
