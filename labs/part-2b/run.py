...
class PolicyNet(nn.Module):
  ...
  """
  def __init__(self, grid_x, grid_y, cell_pixel_size, scl_path):
  ...
  def forward(self, x):
    ...
    action_scores = torch.softmax(result["next_action"], dim=1)
    violation = result["violation"] * 0.2
    return action_scores, violation
...
class DQN:
  def __init__(self, grid_x, grid_y, cell_pixel_size, scl_path):
    self.policy_net = PolicyNet(grid_x, grid_y, cell_pixel_size, scl_path)

    # Create another network
    self.target_net = PolicyNet(grid_x, grid_y, cell_pixel_size, scl_path)
  ...
  def optimize_model(self):
    ...
    # Compute the loss:
    loss1 = self.criterion(state_action_values, expected_state_action_values)
    loss2 = self.semantic_loss(violations, torch.zeros(args.batch_size))
    loss = loss1 + loss2
...
class Trainer:
  def __init__(self, model_dir, grid_x, grid_y, cell_size, dpi, num_enemies, epsilon, scl_path):
    self.model_dir = model_dir
    self.arena = AvoidingArena((grid_x, grid_y), cell_size, dpi, num_enemies, easy=args.easy)
    self.dqn = DQN(grid_x, grid_y, self.arena.cell_pixel_size(), scl_path)
    self.epsilon = epsilon

    if args.load_model is not None:
      model_path = os.path.abspath(os.path.join(os.path.abspath(__file__), '../../', args.load_model))
      print(model_path)
      self.dqn.policy_net.extract_entity.load_state_dict(torch.load(model_path))
  ...
  def test_epoch(self, epoch):
    ...
    # Save the model
    if args.save_model:
      torch.save(self.dqn.policy_net.extract_entity.state_dict(), os.path.join(self.model_dir, "entity_extractor.pkl"))
...
if __name__ == "__main__":
  scl_path = os.path.abspath(os.path.join(os.path.abspath(__file__), '../path_planner.scl'))
  ...
  parser.add_argument("--num-test-episodes", type=int, default=200)
  ...
  parser.add_argument("--scl-path", type=str, default=scl_path)
  ...
  # Train
  trainer = Trainer(model_dir, args.grid_x, args.grid_y, args.cell_size, args.dpi, args.num_enemies, args.epsilon, scl_path=args.scl_path)
  if args.phase == "train":
    trainer.run()
  else:
